 ...
 */
class RelCustomerAssembly extends CI_Model{
    ...
    public function add($ProjectID, $VenderID) {
        if (!empty($VenderID)) {
            foreach($VenderID as $get){
                ...
                $this->db->insert('rel_customer_project_assembly', $data);
    ...
    public function edit($ProjectID, $VenderID) {
        $this->db->where('CustomerProjectID', $ProjectID)->delete('rel_customer_project_assembly');
        if (!empty($VenderID)) {
            foreach($VenderID as $get){
                $data = ['CustomerProjectID'=>$ProjectID, 'VenderID'=>$get];
                //set the datetime based on the db driver in use
