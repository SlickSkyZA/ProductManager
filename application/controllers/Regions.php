...
class Regions extends CI_Controller{
    ...
    public function index(){
        $data['pageContent'] = $this->load->view('regions/regions', '', TRUE);
        $data['pageTitle'] = "Region(OEM)";
    ...
    */
    ...
    public function lilt(){
        ...
        $totalItems = $this->db->count_all('customer_region');
        ...
        $config = $this->genlib->setPaginationConfig($totalItems, "regions/lilt", $limit, ['onclick'=>'return lilt(this.href);']);

        $this->pagination->initialize($config);//initialize the library class

        //get all items from db
        $data['allItems'] = $this->region->getAll($orderBy, $orderFormat, $start, $limit);
        $data['range'] = $totalItems > 0 ? "Showing " . ($start+1) . "-" . ($start + count($data['allItems'])) . " of " . $totalItems : "";
        $data['links'] = $this->pagination->create_links();//page links
        $data['sn'] = $start+1;

        $json['itemsListTable'] = $this->load->view('regions/regionslisttable', $data, TRUE);//get view with populated items table
    ...
    */
    ...
    public function add(){
        ...
        $this->form_validation->set_rules('fullName', 'Region Name(Full)', ['required', 'trim', 'max_length[80]', 'is_unique[priority.Value]'], //numeric
            ['required'=>"required"]);
        $this->form_validation->set_rules('shortName', 'Region Name(Short)', ['required', 'trim', 'max_length[80]', 'is_unique[priority.Value]'],
            ['required'=>"required"]);
        ...
        if($this->form_validation->run() !== FALSE){
             ...
             */
            $insertedId = $this->region->add(set_value('fullName'), set_value('shortName'));

            $itemName = set_value('fullName');

            //insert into eventlog
            //function header: addevent($event, $eventRowId, $eventDesc, $eventTable, $staffId)
            $desc = "New addition of region:{$itemName}";

            $insertedId ? $this->genmod->addevent("Creation of new region", $insertedId, $desc, "Cusomer Region", $this->session->admin_id) : "";

            $this->db->trans_complete();

            $json = $this->db->trans_status() !== FALSE ?
                    ['status'=>1, 'msg'=>"Cusomer Region successfully added"]
                    :
