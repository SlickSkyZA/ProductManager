...
$(document).ready(function(){
    ...
    $("#addNewItem").click(function(e){
        ...
        changeInnerHTML(['venderNameErr', 'addCustErrMsg'], "");

        var venderName = $("#venderName").val();
        var description = $("#description").val();

        if(!venderName){
            !venderName ? $("#venderNameErr").text("required") : "";
        ...
        displayFlashMsg("Adding Vender '"+venderName+"'", "fa fa-spinner faa-spin animated", '', '');

        $.ajax({
            type: "post",
            url: appRoot+"customerVenders/add",
            data:{venderName:venderName, description:description},
            ...
            success: function(returnedData){
                if(returnedData.status === 1){
                    ...
                    //display all errors
                    $("#venderNameErr").text(returnedData.venderName);
    ...
    $("#itemSearch").keyup(function(){
        ...
        if(value){
            $.ajax({
                url: appRoot+"search/customerVenderSearch",
                type: "get",
                data: {v:value},
                success: function(returnedData){
    ...
    $("#editItemSubmit").click(function(){
        ...
        $.ajax({
            method: "POST",
            url: appRoot+"customerVenders/edit",
    ...
    $("#itemsListTable").on('click', '.delItem', function(e){
        ...
        if(itemId){
            ...
            if(confirm){
                ...
                $.ajax({
                    url: appRoot+"customerVenders/delete",
                    method: "POST",
                    data: {i:itemId}
                }).done(function(rd){
                    if(rd.status === 1){
...
function lilt(url){
    ...
    $.ajax({
        type:'get',
        url: url ? url : appRoot+"customerVenders/lilt/",
