...
$(document).ready(function(){
    ...
    $("#addNewItem").click(function(e){
        ...
        changeInnerHTML(['statusNameErr', 'addCustErrMsg'], "");

        var statusName = $("#statusName").val();
        var description = $("#description").val();

        if(!statusName){
            !statusName ? $("#statusNameErr").text("required") : "";
        ...
        displayFlashMsg("Adding Status '"+statusName+"'", "fa fa-spinner faa-spin animated", '', '');

        $.ajax({
            type: "post",
            url: appRoot+"productStatuses/add",
            data:{statusName:statusName, description:description},
            ...
            success: function(returnedData){
                if(returnedData.status === 1){
                    ...
                    //display all errors
                    $("#statusNameErr").text(returnedData.statusName);
    ...
    $("#itemSearch").keyup(function(){
        ...
        if(value){
            $.ajax({
                url: appRoot+"search/productStatusSearch",
                type: "get",
                data: {v:value},
                success: function(returnedData){
    ...
    $("#editItemSubmit").click(function(){
        ...
        $.ajax({
            method: "POST",
            url: appRoot+"productStatuses/edit",
    ...
    $("#itemsListTable").on('click', '.delItem', function(e){
        ...
        if(itemId){
            ...
            if(confirm){
                ...
                $.ajax({
                    url: appRoot+"productStatuses/delete",
                    method: "POST",
                    data: {i:itemId}
                }).done(function(rd){
                    if(rd.status === 1){
...
function lilt(url){
    ...
    $.ajax({
        type:'get',
        url: url ? url : appRoot+"productStatuses/lilt/",
